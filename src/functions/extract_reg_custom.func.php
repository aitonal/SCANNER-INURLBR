...
function __extractRegCustom($html, $url_) {
    ...
    $matches = NULL;
        ...
        echo "{$_SESSION["c1"]}[ INF ][URL REGEX CUSTOM] {$_SESSION["c0"]}=>{$_SESSION["c9"]} {$url_} {$_SESSION["c0"]}\n";
        $matches_ = array_filter(array_unique(array_unique($matches[0])));
        foreach ($matches_ as $valor):
            if (__not_empty($valor)):
                echo "{$_SESSION["c1"]}[  +  ]{$_SESSION["c0"]}[\033[01;31m {$_SESSION['config']['cont_valores']} {$_SESSION["c0"]}] {$valor}\n";
