...
function __process($resultadoURL) {
    ...
    echo "\n{$_SESSION["c1"]}[ INF ] {$_SESSION["c16"]}[ TOTAL FOUND VALUES ]::{$_SESSION["c1"]} [ {$_SESSION['config']['total_url']} ]{$_SESSION["c0"]}\n";
    ...
    if (count($resultadoURL[0]) > 0):
        ...
        elseif($_SESSION['config']['irc']['my_fork'] == -1):
            __getOut("{$_SESSION["c1"]}[ INF ] {$_SESSION["c0"]}{$_SESSION["c2"]}ERROR Fork failed{$_SESSION["c0"]}\n");
        endif;
