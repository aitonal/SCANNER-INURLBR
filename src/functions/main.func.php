...
function __main($dork, $motor, $cod) {
    ...
    $dork_list[3] = array_filter(array_unique(array_merge($dork_list[0], $dork_list[1], $dork_list[2])));
    ...
    for ($i = 0; $i <= count($dork_list[3]); $i++):
        ...
        if (!empty($dork_list[3][$i])):
            ...
            echo "\n{$cor->whit}[ INF ]{$cor->end}{$cor->red2} [ DORK ]:: {$cor->whit}[ {$dork_list[3][$i]} ]", PHP_EOL;
